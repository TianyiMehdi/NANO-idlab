...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lorenz coupling experiment with NANO filter.")
    ...
    parser_filter.add_argument(
        ...
        default="sigma",
        choices=["sigma", "mc"],
        help="Expectation method for NANO updates",
    )
    parser_filter.add_argument("--mc_samples", default=50, type=int, help="MC samples when mean_method='mc'")
    parser_filter.add_argument(
        "--init_type",
        default="ukf",
        type=str,
        choices=["prior", "laplace", "iekf", "ukf"],
        help="Initialization mode for NANO",
    )

    filter_args = remaining
    args_filter_preview, _ = parser_filter.parse_known_args(filter_args)
    if args_filter_preview.init_type == "iekf":
        parser_filter.add_argument("--iekf_max_iter", default=2, type=int, help="Iterations for iekf init")

    args_filter = parser_filter.parse_args(filter_args)
