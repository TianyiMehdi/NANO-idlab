...
if __name__ == '__main__':
    ...
    1. Box Plot
    '''
    armse_ekf = np.mean(np.sqrt(np.mean((data_files['EKF']['x_mc'][:, 2:, ] -
                                         data_files['EKF']['x_hat_mc'][:, 2:, ]) ** 2, axis=(1))), axis=1)
    armse_ukf = np.mean(np.sqrt(np.mean((data_files['UKF']['x_mc'][:, 2:, ] -
                                         data_files['UKF']['x_hat_mc'][:, 2:, ]) ** 2, axis=(1))), axis=1)
    armse_IEKF = np.mean(np.sqrt(np.mean((data_files['IEKF']['x_mc'][:, 2:, ] -
                                          data_files['IEKF']['x_hat_mc'][:, 2:, ]) ** 2, axis=(1))), axis=1)
    armse_plf = np.mean(np.sqrt(np.mean((data_files['PLF']['x_mc'][:, 2:, ] -
                                         data_files['PLF']['x_hat_mc'][:, 2:, ]) ** 2, axis=(1))), axis=1)
    armse_ggf = np.mean(np.sqrt(np.mean((data_files['NANO']['x_mc'][:, 2:, ] -
                                         data_files['NANO']['x_hat_mc'][:, 2:, ]) ** 2, axis=(1))), axis=1)
    ...
    cleaned_armse_values = [[value for value in arr if value <= 8] for arr in armse_values]
    ...
    # plt.yscale('log')
    box = plt.boxplot(cleaned_armse_values, vert=True, patch_artist=True,
    ...
    means = [np.mean(datapoint) for datapoint in cleaned_armse_values]
    ...
    for tmp in range(len(labels)):  #
        for i in range(num_states):  # x_dim
            ...
            # {'Error': get_error(labels[tmp])[j, :time_length, i],
                                            {'Error': get_error(labels[tmp])[mc_index, 2:time_length, i],
                                            'Algorithm': labels[tmp],
                                            'Step': np.arange(time_length-2),
                                            'Color': colors[tmp]
                                            }))
    ...
    for state_index in range(num_states):
        ...
        for filter_idx, filter_name in enumerate(labels):
            ...
            # 准备用于绘图的数据
            steps = np.arange(time_length-2)

            plt.plot(steps, rmse[2:time_length], label=filter_name, color=colors[filter_idx], linewidth=4)
